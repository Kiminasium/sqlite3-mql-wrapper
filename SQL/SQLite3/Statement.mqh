...
public:
   ...
   ~Statement() {if (isValid())sqlite3_finalize(m_ref);}
    ...
    string getExpandedSql() const {
        return StringFromUtf8Pointer(sqlite3_expanded_sql(m_ref));
    }
    ...
    string getParameterName(int i) const {
        return StringFromUtf8Pointer(sqlite3_bind_parameter_name(m_ref, i));
    }
    ...
    int bind(int i, const uchar &value[], bool copy = true) {
        return sqlite3_bind_blob(m_ref, i, value, ArraySize(value),
    ...
    string getColumnName(int i) const {
        return StringFromUtf8Pointer(sqlite3_column_name(m_ref, i));
    }
    int getColumnType(int i) const {return sqlite3_column_type(m_ref, i);}
    string getColumnDeclareType(int i) const {
        return StringFromUtf8Pointer(sqlite3_column_decltype(m_ref, i));
    }
    int getColumnBytes(int i) const {return sqlite3_column_bytes(m_ref, i);}

    string getColumnDatabaseName(int i) const {
        return StringFromUtf8Pointer(sqlite3_column_database_name(m_ref, i));
    }
    string getColumnTableName(int i) const {
        return StringFromUtf8Pointer(sqlite3_column_table_name(m_ref, i));
    }
    string getColumnOriginName(int i) const {
        return StringFromUtf8Pointer(sqlite3_column_origin_name(m_ref, i));
    }
    ...
    void getColumn(int i, string &value) const {
        ...
        if (u8p > 0) {
            value = StringFromUtf8Pointer(u8p);
        } else {
            value = NULL;
        }
    }
